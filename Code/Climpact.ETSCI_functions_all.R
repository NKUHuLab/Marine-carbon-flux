...
climdex.rxnday <- function(daily_data, n) {
  daily_data$year_month <- format(as.Date(paste(daily_data$year, daily_data$month, daily_data$day, sep="-")), "%Y-%m")
  monthly_max_nday_sum <- tapply(daily_data$prcp, daily_data$year_month, function(month_precip) {
    if (length(month_precip) < n) {
      return(NA)
    }
    moving_sums <- zoo::rollsum(month_precip, k = n, align = "right", fill = NA)
    max_sum <- max(moving_sums, na.rm = TRUE)
    if (is.infinite(max_sum)) {
      return(NA)
    } else {
      return(max_sum)
    }
  })
  return(monthly_max_nday_sum)
}
...
climdex.cdd <- function(daily_data) {
  daily_data$year_month <- format(as.Date(paste(daily_data$year, daily_data$month, daily_data$day, sep="-")), "%Y-%m")
  monthly_max_spells <- tapply(daily_data$prcp, daily_data$year_month, function(month_precip) {
    is_dry <- month_precip < 1.0
    rle_results <- rle(is_dry)
    dry_spell_lengths <- rle_results$lengths[rle_results$values == TRUE]
    if (length(dry_spell_lengths) == 0) {
      return(0)
    } else {
      return(max(dry_spell_lengths))
    }
  })
  return(monthly_max_spells)
}
...
climdex.cwd <- function(daily_data) {
  daily_data$year_month <- format(as.Date(paste(daily_data$year, daily_data$month, daily_data$day, sep="-")), "%Y-%m")
  monthly_max_spells <- tapply(daily_data$prcp, daily_data$year_month, function(month_precip) {
    is_wet <- month_precip >= 1.0
    rle_results <- rle(is_wet)
    wet_spell_lengths <- rle_results$lengths[rle_results$values == TRUE]
    if (length(wet_spell_lengths) == 0) {
      return(0)
    } else {
      return(max(wet_spell_lengths))
    }
  })
  return(monthly_max_spells)
}
...
climdex.r95p <- function(ci) {
  ...
  return(climdex.pcic::total.precip.op.threshold(ci@data$prec,
                                                 ci@date.factors$monthly,
                                                 ci@quantiles$prec["q95"], ">") * ci@namasks$monthly$prec)
}
...
climdex.r99p <- function(ci) {
  ...
  return(climdex.pcic::total.precip.op.threshold(ci@data$prec,
                                                 ci@date.factors$monthly,
                                                 ci@quantiles$prec["q99"], ">") * ci@namasks$monthly$prec)
}
...
climdex.hw <- function(ci, min.base.data.fraction.present, ehfdef = "NF13") {
    ...
    if (any(is.null(ci@quantiles$tmax$outbase$q90_15days), is.null(ci@quantiles$tmin$outbase$q90_15days), is.null(ci@quantiles$tavg$outbase$q90_15days))) {
        ...
        tavg90p <- suppressWarnings(get.outofbase.quantiles(tavg, ci@data$tmin,
    ...
    if (ehfdef == "NF13") {
        ...
        annualrepeat_tavg90 <- tavg90p[match(fact2, fact2)]
    }
...
get.hw.aspects <- function(aspect.array, boolean.str, yearly.date.factors, monthly.date.factors, daily.data, northern.hemisphere, ehfdef, ehf = FALSE, ecf = FALSE, namask = 1) {
    ...
    for (year in levels(yearly.date.factors)[1]:levels(yearly.date.factors)[nyears]) {
        if ((ehf == TRUE && ehfdef == "NF13") || ecf == TRUE) {
            summer_indices <- which(yearly.date.factors %in% as.factor(year))
        } else {
        ...
        if (!is.na(nhw) && nhw > 0) {
            ...
            for (i in 1:nhw) { # over each run
                ...
                if (ecf == TRUE) {
                    hwa[i] <- min(extended_data[i1:i2], na.rm = TRUE)
                } else {
        ...
        if (is.nan(hwm2) || is.na(hwm2)) {
            ...
            if (ecf == TRUE) {
                aspect.array[2, aspect_ind] <- hwa[which.min(hwm)]
            } else {
    ...
    if (northern.hemisphere == FALSE) {
        if (all(ehf, ehfdef == "NF13") || ecf) { } else {
